...
* @author monet 
*/
public class DaoVendaCancelamentoCartao2 extends ConexaoMySql {
static Conectar cc = new Conectar();
    ...
    * return int
    */
    public int salvarvendaDAO(ModelVendaCancelamentoCartao2 pModelvenda){
        try {
            this.conectar();
            return this.insertSQL(
                "INSERT INTO tbl_venda_cancelamento_cartao2 ("
                    + "fk_cliente, "
                    + "fk_usuario, "
                    + "data, "
                    + "hora, "
    ...
    * return Modelvenda
    */
    public ModelVendaCancelamentoCartao2 getvendaDAO(int pIdVenda){
        ModelVendaCancelamentoCartao2 modelvenda = new ModelVendaCancelamentoCartao2();
        try {
            this.conectar();
            this.executarSQL(
                "SELECT "
                   + "pk_id_venda,"
                    ...
                    + "fk_venda "
                    + " FROM"
                     + " tbl_venda_cancelamento_cartao2 "
                    + " WHERE"
    ...
    * recupera uma lista de venda
        * return ArrayList
    */
    public ArrayList<ModelVendaCancelamentoCartao2> getListavendaDAO(){
        ArrayList<ModelVendaCancelamentoCartao2> listamodelvenda = new ArrayList();
        ModelVendaCancelamentoCartao2 modelvenda = new ModelVendaCancelamentoCartao2();
        try {
            this.conectar();
            this.executarSQL( 
            ...
            while(this.getResultSet().next()){
                modelvenda = new ModelVendaCancelamentoCartao2();
                modelvenda.setIdVenda(this.getResultSet().getInt(1));
                modelvenda.setCliente(this.getResultSet().getInt(2));
                modelvenda.setUsuario(this.getResultSet().getInt(3));
                modelvenda.setData(this.getResultSet().getString(4));
    ...
    * return boolean
    */
    public boolean atualizarvendaDAO(ModelVendaCancelamentoCartao2 pModelvenda){
    try {
    this.conectar();
    return this.executarUpdateDeleteSQL("UPDATE tbl_venda_cancelamento_cartao2 SET "
    ...
    */
    public boolean excluirvendaDAO(int pIdVenda){
        try {
            ...
            return this.executarUpdateDeleteSQL(
                "DELETE FROM tbl_venda_cancelamento_cartao2 "
                + " WHERE "
                    + "pk_id_venda = '" + pIdVenda + "'"
                + ";"
            );
    ...
    public static int eliminaTodos() {
        ...
        int rsu = 0;
        String sql = ModelVendaCancelamentoCartao2.ELIMINAR_TUDO;
        try {
    ...
    public boolean salvarListaVendaProdutoDAO(ArrayList<ModelVendaCancelamentoCartao2> pListaModelVendaProduto) {
    try {
            this.conectar();
